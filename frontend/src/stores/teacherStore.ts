...
export const useTeacherStore = create<TeacherState>()((set, get) => ({
  ...
  fetchTeachers: async () => {
    ...
    try {
      ...
      const data = await api.get<{ items: Teacher[]; total: number }>(`/teachers?${params}`);
  ...
  setSearchTeacherNo: (searchTeacherNo) => {
    set({ searchTeacherNo, page: 1 });
    get().fetchTeachers();
  },
  ...
  updateTeacher: async (teacher_no, data) => {
    await api.put(`/teachers/${teacher_no}`, data);
    await get().fetchTeachers();
    get().closeModal();
  },

  deleteTeacher: async (teacher_no) => {
    await api.delete(`/teachers/${teacher_no}`);
    await get().fetchTeachers();
  },
}));