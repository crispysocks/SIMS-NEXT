...
from app.tutor.subjects.math.knowledge import (
    TOPIC_TO_KNOWLEDGE,
    TOPIC_TO_OBJECTIVES,
)


# -- Safety wrappers (unchanged) -----------------------------------------------
...
class MathQuestionEngine(SubjectEngine):
    """Deterministic math question generator backed by SymPy.

    Implements the SubjectEngine interface for the math subject.
    All 15 question templates (5 topics x 3 difficulties) are preserved
    exactly from the original QuestionEngine.
    ...
    _TOPICS = ("linear_equation", "quadratic_equation", "factoring", "derivative", "integral")
    ...
    def validate(self, student_answer: str, correct_answer: str) -> ValidationResult:
        ok = safe_compare(student_answer, correct_answer)
    ...
    def _register_all(self) -> None:
        """Register all 15 template methods (5 topics x 3 difficulties)."""
    ...
    def _linear_hard(self) -> Question:
        ...
        return Question(
            ...
            difficulty="hard",
            question_text=(
                f"Solve: ${a}x {sign_b} {abs(b)} = {cx_part} + {abs(d)}$"
                if d >= 0
